...
bool LoadMalSite(void);
int CheckMalSite(unsigned char*);
int main(int argc, char* argv[]) {
	...
	if (LoadMalSite() == false) {
		printf("error in Load Malsite.txt\n");
		return -1;
	}
...
int CheckMalSite(unsigned char* pcap_data) {
	char *pos, chr, site[BUFSIZ] = { 0, };

	pos = strstr((char*)pcap_data, "Host: ");
	if (pos == NULL) {
	...
	for (int i = 0; i < URLNUM; i++) {
		if (!strcmp((char*)MalSite[i], site)) {
			printf("[!] Detected Accessing MalSite!\n");
			printf("MalSite : %s\n", site);
			return 1;
		}
	}
